...
\fancyhead[L]{\nouppercase{\leftmark}}
\fancyhead[R]{\thepage}
\fancyfoot[C]{\footnotesize Uniwersytet Rzeszowski \textbullet\ Projekt Inżynierski \textbullet\ 2024/2025}
    ...
    \begin{tikzpicture}[remember picture,overlay]
        ...
        \node at ([yshift=4.5cm]current page.south) {
            \begin{tikzpicture}
                ...
                \foreach \tech/\x in {React/0, .NET/2.7, TypeScript/5.4, PostgreSQL/8.1, Docker/10.8} {
...
\end{center}
\vspace{2cm}
\noindent Ja, niżej podpisany Jan Kowalski, student Instytutu Informatyki Uniwersytetu Rzeszowskiego, oświadczam, że niniejsza praca została wykonana przeze mnie samodzielnie i nie zawiera treści uzyskanych w sposób niezgodny z obowiązującymi przepisami.

\vspace{1.5cm}
\noindent Oświadczam również, że przedstawiona praca nie była wcześniej przedmiotem procedur związanych z uzyskaniem zaliczenia na tej lub innej uczelni.
...
Niniejsza dokumentacja przedstawia aplikację webową do zarządzania budżetem osobistym, która umożliwia użytkownikom śledzenie dochodów i wydatków, kategoryzowanie transakcji, tworzenie miesięcznych budżetów oraz analizę nawyków finansowych w czasie rzeczywistym.

Projekt został zrealizowany w architekturze klient-serwer z wykorzystaniem technologii .NET 8 na backendzie oraz React 18 z TypeScript na frontendzie. Aplikacja korzysta z bazy danych PostgreSQL 15, Redis do cachowania oraz jest w pełni skonteneryzowana (Docker + Docker Compose). Wdrożenie jest w pełni zautomatyzowane za pomocą pipeline'u CI/CD w GitHub Actions.

Dokumentacja zawiera szczegółowy opis architektury (zgodnej z zasadami Clean Architecture), modelu danych, implementacji kluczowych funkcjonalności, dokumentacji REST API, przeprowadzonych testów jednostkowych i end-to-end oraz zastosowanych praktyk DevOps i mechanizmów bezpieczeństwa.

\vspace{1cm}
\noindent\textbf{Słowa kluczowe:} finanse osobiste, budżet domowy, React, .NET 8, TypeScript, PostgreSQL, Docker, Clean Architecture, CQRS, JWT, Redis.
...
\include{rozdziały/wprowadzenie}
\include{rozdziały/architektura}
\include{rozdziały/implementacja}
\include{rozdziały/frontend}
\include{rozdziały/wdrozenie}
\include{rozdziały/wnioski}
