...
Warstwa kliencka systemu została zbudowana przy użyciu biblioteki \textbf{React 19} \cite{react_meta}. Do budowy i optymalizacji projektu wykorzystano narzędzie Vite \cite{vite_guide}. Poniżej opisano technologiczną realizację kluczowych modułów oraz funkcjonalności interfejsu użytkownika, skupiając się na logice biznesowej i integracji z mikroserwisami za pomocą biblioteki Axios \cite{axios_docs}.
...
Użytkownik po zalogowaniu ma dostęp do pulpitu (\textit{Dashboard}), który agreguje informacje o nadchodzących wizytach. System umożliwia również zarządzanie danymi profilowymi, wykorzystując zaawansowane funkcje normalizacji danych wejściowych, co gwarantuje spójność bazy danych. Testowanie logiki komponentów frontendowych zrealizowano przy pomocy środowiska Vitest \cite{vitest_guide}.
...
Panel administracyjny firmy dynamicznie dostosowuje zakres dostępnych funkcjonalności na podstawie roli użytkownika (RBAC – \textit{Role-Based Access Control}). Logika frontendu rozróżnia uprawnienia właściciela (\textit{Owner}), managera oraz pracownika, korzystając z informacji zaszyfrowanych w standardzie JSON Web Token \cite{jwt_rfc}.
...
Ważną funkcjonalnością jest wizualizacja czasu pracy. Użytkownik może definiować dni i godziny pracy oraz konfigurować przerwy (\textit{Staff Breaks}). Do obsługi powiadomień o zmianach w czasie rzeczywistym przewidziano integrację z technologią SignalR \cite{signalr_microsoft}.
...
Administratorzy systemowi zarządzają szablonami wiadomości, które wspierają dynamiczne wstrzykiwanie zmiennych. Pozwala to na personalizację powiadomień e-mail i SMS wysyłanych przez mikroserwis powiadomień zintegrowany z systemem kolejek RabbitMQ \cite{rabbitmq_docs}.
...
W widoku szczegółów usługi zaimplementowano interaktywną mapę \textbf{OpenStreetMap}. System przelicza adres tekstowy na współrzędne geograficzne i wyświetla lokalizację salonu, co znacząco poprawia użyteczność aplikacji. Całość procesu wdrażania frontendu jako kontenera została opisana w dokumentacji Docker \cite{docker_docs}.
