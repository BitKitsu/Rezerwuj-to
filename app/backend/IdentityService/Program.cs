...
using (var scope = app.Services.CreateScope())
{
        ...
        try
        {
            ...
            try
            {
                dbContext.Database.ExecuteSqlRaw(
                    "CREATE TABLE IF NOT EXISTS \"EmailVerificationCodes\" (\"Id\" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \"UserId\" text NOT NULL, \"Email\" text NOT NULL, \"CodeHash\" text NOT NULL, \"FailedAttempts\" integer NOT NULL DEFAULT 0, \"CreatedAt\" timestamp without time zone NOT NULL, \"ExpiresAt\" timestamp without time zone NOT NULL);");
                dbContext.Database.ExecuteSqlRaw(
                    "CREATE UNIQUE INDEX IF NOT EXISTS \"UX_EmailVerificationCodes_UserId\" ON \"EmailVerificationCodes\" (\"UserId\");");
                dbContext.Database.ExecuteSqlRaw(
                    "CREATE INDEX IF NOT EXISTS \"IX_EmailVerificationCodes_Email\" ON \"EmailVerificationCodes\" (\"Email\");");
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Failed to ensure EmailVerificationCodes schema.");
            }
    ...
    for (int retry = 0; retry < maxRetryCount; retry++)
    {
        try
        {
            ...
            var roleNames = new[] { "Admin", "User", "CompanyOwner", "CompanyManager" };
